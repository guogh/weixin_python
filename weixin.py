...
def main():
    ...
    with open('alldata.csv', 'w', newline='') as csvfile:
        ...
        spamwriter.writerow(MemberList)
    ...
    title = ['','备注名字','城市','性别','星标好友','个性签名','微信名','微信号','nickName']
    for i, col in enumerate(title):
        booksheet.write(0, i, col)
    ...
    for Member in MemberList:
        ...
        remark = 'noremark' if remark == '' else remark #备注
        alias = Member['Alias']
        alias = 'noalias' if alias == '' else alias #微信号
        nick = Member['NickName']
        nick = 'nonick' if nick == '' else nick
        print(name,'  ^+*+^  ',city,'  ^+*+^  ',Member['Sex'],' ^+*+^ ',Member['StarFriend'],' ^+*+^ ',sign,' ^+*+^ ',remark,' ^+*+^ ',alias,' ^+*+^ ',nick )
        excelLine=['',remark,city,Member['Sex'],Member['StarFriend'],sign,name,alias,nick]
        for i, col in enumerate(excelLine):
            booksheet.write(imageIndex, i, col)

        allname = name if remark=='noremark' else remark

        strinfo = re.compile('(\")|(<)|(>)|(/)|(' ')')
        allname = strinfo.sub('',allname)

        name = '/Users/guogh/Desktop/friendImage/image'+str(imageIndex)+'('+ str(allname) +')'+'.jpg'
